...
typedef enum{
    r_date, r_partenza, r_capolinea, r_codice, r_ricercalin, r_ricercadic, r_fine, r_errore
}comando_e;
typedef struct {
    char codTratta[MAX];
    char partenza[MAX];
...
int main() {
    ...
    int N,i,command=-1;
    FILE *in;
    Tab automezzo[MAXRIGHE];
    if ((in=fopen("../corse.txt","r"))==NULL){
...
int leggiComando(){
    ...
    char comando[12];
    char tabella[7][12] = {
            "date", "partenza", "capolinea",  "cod", "lineare", "dicotomica",  "fine"
    };
    scanf("%s",comando);
    c=r_date;
...
void  menuParola(Tab autom[], int c, int N) {
    ...
    switch (c) {
        case r_date:
            ...
            scanf("  %c", &rule);
            //il \n dal precedente printf veniva salvato in rule (qualcosa sul buffer)
            //ricordarsi di scrivere spazio prima del %c per eliminare residui di vecchi printf
            if(rule=='Y'){
                stampa(N,orddata);
            }
            break;
        ...
        case r_ricercadic:
            ...
            ordinaPartenza(ordpartenza,N);
            //non ha senso non ordinare se il vettore era preordinato dato che i dati precedenti vengono distrutti
...
void ordinaDate(Tab *pautom[], int N){
    ...
    for(i=1;i<N;i++){
        ...
        while(  (j>=0 && strcmp(altrotemp->data,pautom[j]->data)<0)   ||   ( j>=0 && (strcmp(altrotemp->orapartenza,pautom[j]->orapartenza)<0) && (strcmp(altrotemp->data,pautom[j]->data)==0) )   ){
            //while(  (j>=0 && strcmp(temp,pautom[j]->data)<0)   ||   ( j>=0 && (strcmp(puntatoreno3,pautom[j]->orapartenza)<0) && (strcmp(temp,pautom[j]->data)==0) )   ){ //se è uguale a quella precedente verrà messa DOPO
            pautom[j+1]=pautom[j];
            j--;
        }
...
int ricercaLineare(Tab *partenza[], int N, Tab *indici[]){
    ...
    for(i=0;i<N;i++){
        if(strstr(partenza[i]->partenza,search)!=NULL) {
            //search andava out of bound su stringa partenza
            indici[k] = partenza[i];
            k++;
        }
    }
...
int partition(Tab *pautom[], int inizio, int fin){
    ...
    while(i<j){ //vedi slide per versione con for(;;) //confronto con la versione non incrementata
        while(strcmp(pautom[i]->codTratta,pivot->codTratta)<0){
            i++; 
        }
        //++i == prima incremento i e poi lo uso
